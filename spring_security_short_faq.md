...
Сервер не знает, кто делает запрос.
...
Название `Unauthorized` историческое и немного вводит в заблуждение.
Фактически этот код означает **Not Authenticated**.
...
Сервер знает пользователя, но у него **недостаточно прав доступа**.
...
И тогда HTTP-коды читаются так:

401 → authentication failed
403 → authorization failed


### Почему название 401 такое странное

Исторически HTTP-стандарт выбрал название:

401 Unauthorized

но фактически он означает:

Unauthenticated

Это признанная путаница. Даже в документации часто пишут:

401 = unauthenticated
403 = unauthorized


### Мини-шпаргалка (идеальная)
